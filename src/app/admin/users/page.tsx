    ...
    <main className="p-2 font-sans bg-gray-100 min-h-screen">
      ...
      <section className="grid grid-cols-3 gap-4 mb-4">
        ...
        <article className="p-3 bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 border-l-4 border-green-500">
          <h2 className="text-lg font-semibold text-gray-800">Staff Users</h2>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase">Level</th>
              <th className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase">Created At</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {currentUsers?.map(user => (
              <tr key={user.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 text-sm text-center">{user.level}</td>
      ...
      <div className="flex justify-between items-center mt-4 bg-white p-3 rounded-lg shadow-md text-sm">
        <span className="text-gray-600">
          ...
          of
          {" "}
          {Math.ceil((filteredUsers?.length ?? 0) / itemsPerPage)}
        </span>
        <div className="flex gap-1">
          ...
          <button
            ...
            className="px-3 py-1 rounded bg-gray-500 text-white text-sm hover:bg-gray-600 disabled:opacity-50"
            disabled={endIndex >= (filteredUsers?.length ?? 0)}
          >
            {">"}
          </button>
          <button
            onClick={() => setCurrentPage(Math.ceil((filteredUsers?.length ?? 0) / itemsPerPage))}
            className="px-3 py-1 rounded bg-gray-500 text-white text-sm hover:bg-gray-600 disabled:opacity-50"
            disabled={endIndex >= (filteredUsers?.length ?? 0)}
          >
            Last
